...
public:
	...
	const char *GetSzProfile() {
		return m_strProfile.GetBuffer(); }
	const char *GetSzProfileNameOnly() {
		return m_strProfile_nameonly.GetBuffer(); }
	...
	bool WriteString(const char *szSect, const int iKey, const char *szVal) {
		return WriteString(szSect, itostring(iKey).GetBuffer(), szVal); }
	...
	//	return WriteInt(szSect, itostring(iKey).GetBuffer(), val); }
	...
	//	return WriteBool(szSect, itostring(iKey).GetBuffer(), val); }
	bool WriteBytes(const char *szSect, const char *szKey, const void *pval, int size);

	CString ReadString(const char *szSect, const char *szKey, const char *def="");
	CString ReadString(const char *szSect, const int iKey, const char *def="") {
		return ReadString(szSect, itostring(iKey).GetBuffer(), def); }
	int ReadInt(const char *szSect, const char *szKey, int def = 0);
	//int ReadInt(const char *szSect, const int iKey, int def = 0) {
	//	return ReadInt(szSect, itostring(iKey).GetBuffer(), def); }
...
protected:
	...
	DWORD m_sizefileArc;
	CString itostring(int i, int base=10)
	{
		char sz[256];
		_itoa_s(i, sz, sizeof(sz),base);
		return CString(sz);
	}
};
