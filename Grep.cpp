...
BOOL CGrepEngin_SIMPLE::InitEngin()
{
	...
	if (m_fMetaChars)
		...
		cchMatched=0;
		int iBlock=0;
		while ( (sl = vpKeyOutLogic->ScanStrNext(pGrepPattern, cchMatched, iBlock==0/*takaso2010*/))!=slEndBlock)
			{
			ASSERT(sl==slNextBlock || sl == slNextPat);
			// パターンを解釈
			if (InitPattern(pGrepPattern , FALSE/*takaso2010*/, iBlock==0)==FALSE)
				return FALSE;
			iBlock++;
		...
		if (iBlock==1){
			...
			if (cstrKeyWord.FindOneOf(cstrCheck)==-1) {
				...
				//再度イニシャライズする
				ASSERT(vpKeyOutLogic->Initialize(cstrKeyWord.GetBuffer(0)));
	...
	if (m_fMetaChars)
		{		
		ScanLogic sl;
		CGrepPattern *pGrepPattern=NULL;
		cchMatched=0;
		int iBlock=0;
		while ( (sl = m_pKeyOutLogic->ScanStrNext(pGrepPattern, cchMatched, iBlock==0/*takaso2010*/))!=slEndBlock)
			{
			ASSERT(sl==slNextBlock || sl == slNextPat);
			// パターンを解釈
			if (InitPattern(pGrepPattern , iBlock==iBlock_bak, iBlock==0/*takaso2010*/)==FALSE)
				return FALSE;
			iBlock++;
		 	cchMatched += pGrepPattern->m_dcpMatchAny;
			}
		if (m_fMetaChars)
			...
			cchMatched=0;
			BOOL ffirst=true;
			while ( (sl = m_pKeyOutLogic->ScanStrNext(pGrepPattern, cchMatched, ffirst/*takaso2010*/))!=slEndBlock)
				{
				ASSERT(sl==slNextBlock || sl == slNextPat);
				//B&Mテーブルの作成
...
// "abc[def^ghi]jkm
// "abc[def!ghi]jkm
BOOL CGrepEngin_SIMPLE::InitPattern(CGrepPattern  *pGrepPattern, BOOL fStripWild_left, BOOL fStripWild_right)
{
	// fStripWild=?ABC?や*ABC*など左右端がマッチング・メタキャラクタである場合は、それを削除する
	if (m_fMetaChars)
		{
		CDString cdstr = pGrepPattern->m_cstr;
		WORD wch;
		if (fStripWild_left)//takaso2010
		{
			//左のマッチング・メタキャラクタの削除
		...
		if (fStripWild_right)//takaso2010 
		{
			//右のマッチング・メタキャラクタの削除
...
//bugfix takaso2010		dcp >= 0; iwPattern++, dcp--)
			dcp >= cchMatched; iwPattern++, dcp--)
		{
...
//　前処理
	...
	if (m_fMetaChars)
		...
		m_pKeyOutLogic->ScanStrNext(pGrepPattern, cchMatchedIO = 0 /*initial*/, true/*takaso2010*/);
		}
	else
		pGrepPattern = m_pGrepPattern;
	int indexLogicCan = -1; // ｷｬﾝｾﾙ･ｲﾝﾃﾞｯｸｽﾛｼﾞｯｸを指す
	long ccpRemainCan = 0;				// ｷｬﾝｾﾙ･ﾄﾞｷｭﾒﾝﾄｽｷｬﾝ残りｻｲｽﾞ
#endif
	...
	if (vpFindDlg->m_fDispDoc || vpFindDlg->m_optDispNum>0)
		...
		if (m_fMetaChars)
			...
			if (rlLine==rlTrue)
				...
				//	return FALSE;	// もうこれ以上リストに入れられない。
				if (!vpFindList->FAddList(m_pcFiles, lMatchFirst, cchMatched, lpchDoc, rlNutral))
					return FALSE;	// もうこれ以上リストに入れられない。
			...
			switch(slTmp)
				...
				case slNextPat:
					//lpchDoc--;
					//ccpRemain++;
					if (wSpecial!=wSpecialStr) {//takaso2010
						lpchDoc--;
						ccpRemain++;
					}
					if (cchMatchedIO != cchMatched)
...
#endif //PATTERNS
		if (!vpFindList->FAddList(m_pcFiles, lMatchFirst, cchMatched, lpchDoc, rlTrue))
			return FALSE;	// もうこれ以上リストに入れられない。
		// 次のを見つけに行く場合のみの処理
		SetForNext(cchMatched, lpchDoc, ccpRemain, cchMatched, pwPat, pwPatLast);
		goto LFCheckRemain;
